...
foreach ($shim in $Shims) {
    ...
    $clArgs = "/LD /O2 /W2 /nologo `"$ScriptDir\$SrcFile`" /link /DEF:`"$ScriptDir\$DefFile`" /OUT:`"$ScriptDir\$OutDll`""
    ...
    foreach ($profile in @("debug", "release")) {
        $dest = Join-Path $RepoRoot "target\$profile\$OutDll"
        if (Test-Path (Split-Path $dest -Parent)) {
            Copy-Item "$ScriptDir\$OutDll" $dest -Force
            Write-Host "已复制到 target\$profile\" -ForegroundColor DarkGray
        }
    }
}
